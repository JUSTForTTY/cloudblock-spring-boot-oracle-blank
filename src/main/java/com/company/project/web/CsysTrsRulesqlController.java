...
public class CsysTrsRulesqlController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysTrsRulesqlBiz csysTrsRulesqlBiz;
    ...
    @RequestBody CsysTrsRulesql csysTrsRulesql) {
        ...
        String insertId=csysTrsRulesqlBiz.insertDataSettings(baseUserList.get(0),csysTrsRulesql);
    ...
    @PathVariable String ids) {
        ...
        csysTrsRulesqlBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysTrsRulesql csysTrsRulesql) {
        ...
        csysTrsRulesqlBiz.updateDataSettings(baseUserList.get(0),csysTrsRulesql);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysTrsRulesql csysTrsRulesql = csysTrsRulesqlBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsRulesql,param);
    ...
    @RequestBody CsysTrsRulesql csysTrsRulesql) {
        ...
        List<CsysTrsRulesql> csysTrsRulesqllst = csysTrsRulesqlBiz.getDataSettingsByCondition(csysTrsRulesql);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsRulesqllst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysTrsRulesqlBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysTrsRulesql csysTrsRulesql) {
        ...
        try{
         PageInfo pageInfo=csysTrsRulesqlBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysTrsRulesql);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
