...
*/
@RestController
@RequestMapping("/csysexcelimport")
public class CsysExcelImportController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysExcelImportBiz csysExcelImportBiz;
    ...
    @RequestBody CsysExcelImport csysExcelImport) {
        ...
        String insertId=csysExcelImportBiz.insertDataSettings(baseUserList.get(0),csysExcelImport);
    ...
    @PathVariable String ids) {
        ...
        csysExcelImportBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysExcelImport csysExcelImport) {
        ...
        csysExcelImportBiz.updateDataSettings(baseUserList.get(0),csysExcelImport);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysExcelImport csysExcelImport = csysExcelImportBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysExcelImport,param);
    ...
    @RequestBody CsysExcelImport csysExcelImport) {
        ...
        List<CsysExcelImport> csysExcelImportlst = csysExcelImportBiz.getDataSettingsByCondition(csysExcelImport);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysExcelImportlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysExcelImportBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysExcelImport csysExcelImport) {
        ...
        try{
         PageInfo pageInfo=csysExcelImportBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysExcelImport);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
