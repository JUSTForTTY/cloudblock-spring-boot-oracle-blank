...
*/
@RestController
@RequestMapping("/csysorgauth")
public class CsysOrgAuthController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysOrgAuthBiz csysOrgAuthBiz;
    ...
    @RequestBody CsysOrgAuth csysOrgAuth) {
        ...
        String insertId=csysOrgAuthBiz.insertDataSettings(baseUserList.get(0),csysOrgAuth);
    ...
    @PathVariable String ids) {
        ...
        csysOrgAuthBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysOrgAuth csysOrgAuth) {
        ...
        csysOrgAuthBiz.updateDataSettings(baseUserList.get(0),csysOrgAuth);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysOrgAuth csysOrgAuth = csysOrgAuthBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysOrgAuth,param);
    ...
    @RequestBody CsysOrgAuth csysOrgAuth) {
        ...
        List<CsysOrgAuth> csysOrgAuthlst = csysOrgAuthBiz.getDataSettingsByCondition(csysOrgAuth);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysOrgAuthlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysOrgAuthBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysOrgAuth csysOrgAuth) {
        ...
        try{
         PageInfo pageInfo=csysOrgAuthBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysOrgAuth);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
