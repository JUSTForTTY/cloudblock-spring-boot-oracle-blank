...
*/
@RestController
@RequestMapping("/csyspageblock")
public class CsysPageBlockController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysPageBlockBiz csysPageBlockBiz;
    ...
    @RequestBody CsysPageBlock csysPageBlock) {
        ...
        String insertId=csysPageBlockBiz.insertDataSettings(baseUserList.get(0),csysPageBlock);
    ...
    @PathVariable String ids) {
        ...
        csysPageBlockBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysPageBlock csysPageBlock) {
        ...
        csysPageBlockBiz.updateDataSettings(baseUserList.get(0),csysPageBlock);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysPageBlock csysPageBlock = csysPageBlockBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysPageBlock,param);
    ...
    @RequestBody CsysPageBlock csysPageBlock) {
        ...
        List<CsysPageBlock> csysPageBlocklst = csysPageBlockBiz.getDataSettingsByCondition(csysPageBlock);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysPageBlocklst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysPageBlockBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysPageBlock csysPageBlock) {
        ...
        try{
         PageInfo pageInfo=csysPageBlockBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysPageBlock);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
