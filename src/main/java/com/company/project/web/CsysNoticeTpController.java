...
*/
@RestController
@RequestMapping("/csysnoticetp")
public class CsysNoticeTpController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysNoticeTpBiz csysNoticeTpBiz;
    ...
    @RequestBody CsysNoticeTp csysNoticeTp) {
        ...
        String insertId=csysNoticeTpBiz.insertDataSettings(baseUserList.get(0),csysNoticeTp);
    ...
    @PathVariable String ids) {
        ...
        csysNoticeTpBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysNoticeTp csysNoticeTp) {
        ...
        csysNoticeTpBiz.updateDataSettings(baseUserList.get(0),csysNoticeTp);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysNoticeTp csysNoticeTp = csysNoticeTpBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysNoticeTp,param);
    ...
    @RequestBody CsysNoticeTp csysNoticeTp) {
        ...
        List<CsysNoticeTp> csysNoticeTplst = csysNoticeTpBiz.getDataSettingsByCondition(csysNoticeTp);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysNoticeTplst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysNoticeTpBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysNoticeTp csysNoticeTp) {
        ...
        try{
         PageInfo pageInfo=csysNoticeTpBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysNoticeTp);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
