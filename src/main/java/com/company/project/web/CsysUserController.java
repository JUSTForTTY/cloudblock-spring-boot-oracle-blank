...
*/
@RestController
@RequestMapping("/csysuser")
public class CsysUserController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysUserBiz csysUserBiz;
    ...
    @RequestBody CsysUser csysUser) {
        ...
        String insertId=csysUserBiz.insertDataSettings(baseUserList.get(0),csysUser);
    ...
    @PathVariable String ids) {
        ...
        csysUserBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysUser csysUser) {
        ...
        csysUserBiz.updateDataSettings(baseUserList.get(0),csysUser);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysUser csysUser = csysUserBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysUser,param);
    ...
    @RequestBody CsysUser csysUser) {
        ...
        List<CsysUser> csysUserlst = csysUserBiz.getDataSettingsByCondition(csysUser);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysUserlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysUserBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysUser csysUser) {
        ...
        try{
         PageInfo pageInfo=csysUserBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysUser);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
