...
*/
@RestController
@RequestMapping("/csyspagelayout")
public class CsysPageLayoutController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysPageLayoutBiz csysPageLayoutBiz;
    ...
    @RequestBody CsysPageLayout csysPageLayout) {
        ...
        String insertId=csysPageLayoutBiz.insertDataSettings(baseUserList.get(0),csysPageLayout);
    ...
    @PathVariable String ids) {
        ...
        csysPageLayoutBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysPageLayout csysPageLayout) {
        ...
        csysPageLayoutBiz.updateDataSettings(baseUserList.get(0),csysPageLayout);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysPageLayout csysPageLayout = csysPageLayoutBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysPageLayout,param);
    ...
    @RequestBody CsysPageLayout csysPageLayout) {
        ...
        List<CsysPageLayout> csysPageLayoutlst = csysPageLayoutBiz.getDataSettingsByCondition(csysPageLayout);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysPageLayoutlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysPageLayoutBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysPageLayout csysPageLayout) {
        ...
        try{
         PageInfo pageInfo=csysPageLayoutBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysPageLayout);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
