...
*/
@RestController
@RequestMapping("/csysoperatelog")
public class CsysOperatelogController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysOperatelogBiz csysOperatelogBiz;
    ...
    @RequestBody CsysOperatelog csysOperatelog) {
        ...
        String insertId=csysOperatelogBiz.insertDataSettings(baseUserList.get(0),csysOperatelog);
    ...
    @PathVariable String ids) {
        ...
        csysOperatelogBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysOperatelog csysOperatelog) {
        ...
        csysOperatelogBiz.updateDataSettings(baseUserList.get(0),csysOperatelog);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysOperatelog csysOperatelog = csysOperatelogBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysOperatelog,param);
    ...
    @RequestBody CsysOperatelog csysOperatelog) {
        ...
        List<CsysOperatelog> csysOperateloglst = csysOperatelogBiz.getDataSettingsByCondition(csysOperatelog);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysOperateloglst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysOperatelogBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysOperatelog csysOperatelog) {
        ...
        try{
         PageInfo pageInfo=csysOperatelogBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysOperatelog);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
