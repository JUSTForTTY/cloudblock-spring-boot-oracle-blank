...
*/
@RestController
@RequestMapping("/csystrspage")
public class CsysTrsPageController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysTrsPageBiz csysTrsPageBiz;
    ...
    @RequestBody CsysTrsPage csysTrsPage) {
        ...
        String insertId=csysTrsPageBiz.insertDataSettings(baseUserList.get(0),csysTrsPage);
    ...
    @PathVariable String ids) {
        ...
        csysTrsPageBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysTrsPage csysTrsPage) {
        ...
        csysTrsPageBiz.updateDataSettings(baseUserList.get(0),csysTrsPage);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysTrsPage csysTrsPage = csysTrsPageBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsPage,param);
    ...
    @RequestBody CsysTrsPage csysTrsPage) {
        ...
        List<CsysTrsPage> csysTrsPagelst = csysTrsPageBiz.getDataSettingsByCondition(csysTrsPage);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsPagelst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysTrsPageBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysTrsPage csysTrsPage) {
        ...
        try{
         PageInfo pageInfo=csysTrsPageBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysTrsPage);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
