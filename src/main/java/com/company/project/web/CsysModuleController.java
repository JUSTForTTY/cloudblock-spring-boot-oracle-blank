...
*/
@RestController
@RequestMapping("/csysmodule")
public class CsysModuleController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysModuleBiz csysModuleBiz;
    ...
    @RequestBody CsysModule csysModule) {
        ...
        String insertId=csysModuleBiz.insertDataSettings(baseUserList.get(0),csysModule);
    ...
    @PathVariable String ids) {
        ...
        csysModuleBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysModule csysModule) {
        ...
        csysModuleBiz.updateDataSettings(baseUserList.get(0),csysModule);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysModule csysModule = csysModuleBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysModule,param);
    ...
    @RequestBody CsysModule csysModule) {
        ...
        List<CsysModule> csysModulelst = csysModuleBiz.getDataSettingsByCondition(csysModule);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysModulelst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysModuleBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysModule csysModule) {
        ...
        try{
         PageInfo pageInfo=csysModuleBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysModule);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
