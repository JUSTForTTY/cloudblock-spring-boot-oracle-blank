...
*/
@RestController
@RequestMapping("/csysorgpotauth")
public class CsysOrgPotAuthController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysOrgPotAuthBiz csysOrgPotAuthBiz;
    ...
    @RequestBody CsysOrgPotAuth csysOrgPotAuth) {
        ...
        String insertId=csysOrgPotAuthBiz.insertDataSettings(baseUserList.get(0),csysOrgPotAuth);
    ...
    @PathVariable String ids) {
        ...
        csysOrgPotAuthBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysOrgPotAuth csysOrgPotAuth) {
        ...
        csysOrgPotAuthBiz.updateDataSettings(baseUserList.get(0),csysOrgPotAuth);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysOrgPotAuth csysOrgPotAuth = csysOrgPotAuthBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysOrgPotAuth,param);
    ...
    @RequestBody CsysOrgPotAuth csysOrgPotAuth) {
        ...
        List<CsysOrgPotAuth> csysOrgPotAuthlst = csysOrgPotAuthBiz.getDataSettingsByCondition(csysOrgPotAuth);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysOrgPotAuthlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysOrgPotAuthBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysOrgPotAuth csysOrgPotAuth) {
        ...
        try{
         PageInfo pageInfo=csysOrgPotAuthBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysOrgPotAuth);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
