...
*/
@RestController
@RequestMapping("/csystrslogpam")
public class CsysTrsLogPamController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysTrsLogPamBiz csysTrsLogPamBiz;
    ...
    @RequestBody CsysTrsLogPam csysTrsLogPam) {
        ...
        String insertId=csysTrsLogPamBiz.insertDataSettings(baseUserList.get(0),csysTrsLogPam);
    ...
    @PathVariable String ids) {
        ...
        csysTrsLogPamBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysTrsLogPam csysTrsLogPam) {
        ...
        csysTrsLogPamBiz.updateDataSettings(baseUserList.get(0),csysTrsLogPam);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysTrsLogPam csysTrsLogPam = csysTrsLogPamBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsLogPam,param);
    ...
    @RequestBody CsysTrsLogPam csysTrsLogPam) {
        ...
        List<CsysTrsLogPam> csysTrsLogPamlst = csysTrsLogPamBiz.getDataSettingsByCondition(csysTrsLogPam);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsLogPamlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysTrsLogPamBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysTrsLogPam csysTrsLogPam) {
        ...
        try{
         PageInfo pageInfo=csysTrsLogPamBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysTrsLogPam);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
