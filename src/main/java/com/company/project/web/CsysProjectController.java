...
*/
@RestController
@RequestMapping("/csysproject")
public class CsysProjectController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysProjectBiz csysProjectBiz;
    ...
    @RequestBody CsysProject csysProject) {
        ...
        String insertId=csysProjectBiz.insertDataSettings(baseUserList.get(0),csysProject);
    ...
    @PathVariable String ids) {
        ...
        csysProjectBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysProject csysProject) {
        ...
        csysProjectBiz.updateDataSettings(baseUserList.get(0),csysProject);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysProject csysProject = csysProjectBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysProject,param);
    ...
    @RequestBody CsysProject csysProject) {
        ...
        List<CsysProject> csysProjectlst = csysProjectBiz.getDataSettingsByCondition(csysProject);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysProjectlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysProjectBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysProject csysProject) {
        ...
        try{
         PageInfo pageInfo=csysProjectBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysProject);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
