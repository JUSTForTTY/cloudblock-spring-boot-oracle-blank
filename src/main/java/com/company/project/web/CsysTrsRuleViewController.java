...
*/
@RestController
@RequestMapping("/csystrsruleview")
public class CsysTrsRuleViewController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysTrsRuleViewBiz csysTrsRuleViewBiz;
    ...
    @RequestBody CsysTrsRuleView csysTrsRuleView) {
        ...
        String insertId=csysTrsRuleViewBiz.insertDataSettings(baseUserList.get(0),csysTrsRuleView);
    ...
    @PathVariable String ids) {
        ...
        csysTrsRuleViewBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysTrsRuleView csysTrsRuleView) {
        ...
        csysTrsRuleViewBiz.updateDataSettings(baseUserList.get(0),csysTrsRuleView);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysTrsRuleView csysTrsRuleView = csysTrsRuleViewBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsRuleView,param);
    ...
    @RequestBody CsysTrsRuleView csysTrsRuleView) {
        ...
        List<CsysTrsRuleView> csysTrsRuleViewlst = csysTrsRuleViewBiz.getDataSettingsByCondition(csysTrsRuleView);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsRuleViewlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysTrsRuleViewBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysTrsRuleView csysTrsRuleView) {
        ...
        try{
         PageInfo pageInfo=csysTrsRuleViewBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysTrsRuleView);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
