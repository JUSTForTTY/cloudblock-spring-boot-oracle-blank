...
*/
@RestController
@RequestMapping("/csyspbassoc")
public class CsysPbAssocController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysPbAssocBiz csysPbAssocBiz;
    ...
    @RequestBody CsysPbAssoc csysPbAssoc) {
        ...
        String insertId=csysPbAssocBiz.insertDataSettings(baseUserList.get(0),csysPbAssoc);
    ...
    @PathVariable String ids) {
        ...
        csysPbAssocBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysPbAssoc csysPbAssoc) {
        ...
        csysPbAssocBiz.updateDataSettings(baseUserList.get(0),csysPbAssoc);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysPbAssoc csysPbAssoc = csysPbAssocBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysPbAssoc,param);
    ...
    @RequestBody CsysPbAssoc csysPbAssoc) {
        ...
        List<CsysPbAssoc> csysPbAssoclst = csysPbAssocBiz.getDataSettingsByCondition(csysPbAssoc);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysPbAssoclst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysPbAssocBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysPbAssoc csysPbAssoc) {
        ...
        try{
         PageInfo pageInfo=csysPbAssocBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysPbAssoc);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
