...
*/
@RestController
@RequestMapping("/csystrsrule")
public class CsysTrsRuleController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysTrsRuleBiz csysTrsRuleBiz;
    ...
    @RequestBody CsysTrsRule csysTrsRule) {
        ...
        String insertId=csysTrsRuleBiz.insertDataSettings(baseUserList.get(0),csysTrsRule);
    ...
    @PathVariable String ids) {
        ...
        csysTrsRuleBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysTrsRule csysTrsRule) {
        ...
        csysTrsRuleBiz.updateDataSettings(baseUserList.get(0),csysTrsRule);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysTrsRule csysTrsRule = csysTrsRuleBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsRule,param);
    ...
    @RequestBody CsysTrsRule csysTrsRule) {
        ...
        List<CsysTrsRule> csysTrsRulelst = csysTrsRuleBiz.getDataSettingsByCondition(csysTrsRule);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsRulelst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysTrsRuleBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysTrsRule csysTrsRule) {
        ...
        try{
         PageInfo pageInfo=csysTrsRuleBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysTrsRule);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
