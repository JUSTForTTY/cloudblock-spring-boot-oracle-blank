...
*/
@RestController
@RequestMapping("/csysuserrole")
public class CsysUserRoleController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysUserRoleBiz csysUserRoleBiz;
    ...
    @RequestBody CsysUserRole csysUserRole) {
        ...
        String insertId=csysUserRoleBiz.insertDataSettings(baseUserList.get(0),csysUserRole);
    ...
    @PathVariable String ids) {
        ...
        csysUserRoleBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysUserRole csysUserRole) {
        ...
        csysUserRoleBiz.updateDataSettings(baseUserList.get(0),csysUserRole);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysUserRole csysUserRole = csysUserRoleBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysUserRole,param);
    ...
    @RequestBody CsysUserRole csysUserRole) {
        ...
        List<CsysUserRole> csysUserRolelst = csysUserRoleBiz.getDataSettingsByCondition(csysUserRole);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysUserRolelst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysUserRoleBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysUserRole csysUserRole) {
        ...
        try{
         PageInfo pageInfo=csysUserRoleBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysUserRole);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
