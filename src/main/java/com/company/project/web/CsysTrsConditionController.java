...
*/
@RestController
@RequestMapping("/csystrscondition")
public class CsysTrsConditionController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysTrsConditionBiz csysTrsConditionBiz;
    ...
    @RequestBody CsysTrsCondition csysTrsCondition) {
        ...
        String insertId=csysTrsConditionBiz.insertDataSettings(baseUserList.get(0),csysTrsCondition);
    ...
    @PathVariable String ids) {
        ...
        csysTrsConditionBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysTrsCondition csysTrsCondition) {
        ...
        csysTrsConditionBiz.updateDataSettings(baseUserList.get(0),csysTrsCondition);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysTrsCondition csysTrsCondition = csysTrsConditionBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsCondition,param);
    ...
    @RequestBody CsysTrsCondition csysTrsCondition) {
        ...
        List<CsysTrsCondition> csysTrsConditionlst = csysTrsConditionBiz.getDataSettingsByCondition(csysTrsCondition);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsConditionlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysTrsConditionBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysTrsCondition csysTrsCondition) {
        ...
        try{
         PageInfo pageInfo=csysTrsConditionBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysTrsCondition);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
