...
*/
@RestController
@RequestMapping("/csyspagegroup")
public class CsysPageGroupController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysPageGroupBiz csysPageGroupBiz;
    ...
    @RequestBody CsysPageGroup csysPageGroup) {
        ...
        String insertId=csysPageGroupBiz.insertDataSettings(baseUserList.get(0),csysPageGroup);
    ...
    @PathVariable String ids) {
        ...
        csysPageGroupBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysPageGroup csysPageGroup) {
        ...
        csysPageGroupBiz.updateDataSettings(baseUserList.get(0),csysPageGroup);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysPageGroup csysPageGroup = csysPageGroupBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysPageGroup,param);
    ...
    @RequestBody CsysPageGroup csysPageGroup) {
        ...
        List<CsysPageGroup> csysPageGrouplst = csysPageGroupBiz.getDataSettingsByCondition(csysPageGroup);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysPageGrouplst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysPageGroupBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysPageGroup csysPageGroup) {
        ...
        try{
         PageInfo pageInfo=csysPageGroupBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysPageGroup);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
