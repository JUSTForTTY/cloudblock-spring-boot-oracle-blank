 ...
 */
@RestController
@RequestMapping("/cysysbaseuser")
public class CySysBaseUserController {
	private final Log logger = LogFactory.getLog(getClass());

	@Resource
	private CySysBaseUserBiz cySysBaseUserBiz;
	...
	@PostMapping
	public Result insertSettingsData(@RequestHeader(value = "access_token") String access_token,
			@RequestHeader(value = "refresh_token") String refresh_token, @RequestBody CySysBaseUser cySysBaseUser) {
		...
		try {
			...
			String insertId = cySysBaseUserBiz.insertDataSettings(baseUserList.get(0), cySysBaseUser);
	...
	public Result delete(@RequestHeader(value = "access_token") String access_token,
		...
		try {
			...
			cySysBaseUserBiz.deleteDataSettings(ids);
			if (tokenRefreshFlag) {
				param.put("access_token", token);
				param.put("refresh_token", refreshtoken);
	...
	public Result update(@RequestHeader(value = "access_token") String access_token,
		...
		try {
			...
			cySysBaseUserBiz.updateDataSettings(baseUserList.get(0), cySysBaseUser);
			if (tokenRefreshFlag) {
				param.put("access_token", token);
				param.put("refresh_token", refreshtoken);
	...
	public Result detail(@RequestHeader(value = "access_token") String access_token,
		...
		try {
			...
			CySysBaseUser cySysBaseUser = cySysBaseUserBiz.getDataSettings(id);
			if (tokenRefreshFlag) {
				param.put("access_token", token);
				param.put("refresh_token", refreshtoken);

			}
			return ResultGenerator.genSuccessResult(cySysBaseUser, param);
	...
	public Result detailByCondition(@RequestHeader(value = "access_token") String access_token,
		...
		try {
			...
			List<CySysBaseUser> cySysBaseUserlst = cySysBaseUserBiz.getDataSettingsByCondition(cySysBaseUser);
			if (tokenRefreshFlag) {
				param.put("access_token", token);
				param.put("refresh_token", refreshtoken);

			}
			return ResultGenerator.genSuccessResult(cySysBaseUserlst, param);
	...
	public Result list(@RequestHeader(value = "access_token") String access_token,
		...
		try {
			PageInfo pageInfo = cySysBaseUserBiz.getPageDataSettings(page, size);
			if (tokenRefreshFlag) {
				param.put("access_token", token);
				param.put("refresh_token", refreshtoken);
	...
	public Result listByCondition(@RequestHeader(value = "access_token") String access_token,
			@RequestHeader(value = "refresh_token") String refresh_token,
			@RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,
			@RequestBody CySysBaseUser cySysBaseUser) {
		...
		try {
			PageInfo pageInfo = cySysBaseUserBiz.getPageDataSettingsByCondition(page, size, baseUserList.get(0),
					cySysBaseUser);
			if (tokenRefreshFlag) {
				param.put("access_token", token);
				param.put("refresh_token", refreshtoken);
	...
	public Result checkMobile(@RequestParam(defaultValue = "0") Integer page,
			@RequestParam(defaultValue = "0") Integer size, @RequestBody CySysBaseUser cySysBaseUser) {

		Map<String, String> param = new HashMap<>();

		if (null == cySysBaseUser.getCySysBaseUserMobile() || "".equals(cySysBaseUser.getCySysBaseUserMobile())) {
			
			return ResultGenerator.genServerErrorResult(param);
		}
		try {
			PageInfo pageInfo = cySysBaseUserBiz.getPageDataSettingsByCondition(page, size, null, cySysBaseUser);
