...
*/
@RestController
@RequestMapping("/csyspottrscon")
public class CsysPotTrsConController {
    ...
    @RequestBody CsysPotTrsCon csysPotTrsCon) {
		...
		/*--------------start 此段代码可优化成redis-----------------------*/
			...
			baseUserView.setCsysUserRefreshToken(token);

			List<CsysUserView> baseUserList = authloginBiz.getDataSettingsByCondition(baseUserView);

		/*--------------end 此段代码可优化成redis-----------------------*/
		
        String insertId=csysPotTrsConBiz.insertDataSettings(baseUserList.get(0),csysPotTrsCon);
    ...
    @PathVariable String ids) {
        ...
        csysPotTrsConBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysPotTrsCon csysPotTrsCon) {
        ...
        csysPotTrsConBiz.updateDataSettings(baseUserList.get(0),csysPotTrsCon);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysPotTrsCon csysPotTrsCon = csysPotTrsConBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysPotTrsCon,param);
    ...
    @RequestBody CsysPotTrsCon csysPotTrsCon) {
        ...
        List<CsysPotTrsCon> csysPotTrsConlst = csysPotTrsConBiz.getDataSettingsByCondition(csysPotTrsCon);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysPotTrsConlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysPotTrsConBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysPotTrsCon csysPotTrsCon) {
        ...
        try{
         PageInfo pageInfo=csysPotTrsConBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysPotTrsCon);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
