...
*/
@RestController
@RequestMapping("/csysmenuauthview")
public class CsysMenuAuthViewController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysMenuAuthViewBiz csysMenuAuthViewBiz;
    ...
    @RequestBody CsysMenuAuthView csysMenuAuthView) {
        ...
        String insertId=csysMenuAuthViewBiz.insertDataSettings(baseUserList.get(0),csysMenuAuthView);
    ...
    @PathVariable String ids) {
        ...
        csysMenuAuthViewBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysMenuAuthView csysMenuAuthView) {
        ...
        csysMenuAuthViewBiz.updateDataSettings(baseUserList.get(0),csysMenuAuthView);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysMenuAuthView csysMenuAuthView = csysMenuAuthViewBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysMenuAuthView,param);
    ...
    @RequestBody CsysMenuAuthView csysMenuAuthView) {
        ...
        List<CsysMenuAuthView> csysMenuAuthViewlst = csysMenuAuthViewBiz.getDataSettingsByCondition(csysMenuAuthView);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysMenuAuthViewlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysMenuAuthViewBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysMenuAuthView csysMenuAuthView) {
        ...
        try{
         PageInfo pageInfo=csysMenuAuthViewBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysMenuAuthView);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PostMapping("/tree")
    public Result userTreeList(@RequestHeader(value = "access_token") String access_token,@RequestHeader(value = "refresh_token") String refresh_token,
    		@RequestBody CsysUser csysUser) {
		...
		/*---------------------------end 授权验证------------------------*/
 		try{
 			 ...
 			 List<CsysMenuAuthView> csysMenuAuthViewList = csysMenuAuthViewBiz.getDataSettingsTree(csysUser);
        ...
        return ResultGenerator.genSuccessResult(csysMenuAuthViewList,param);
