...
*/
@RestController
@RequestMapping("/csysrole")
public class CsysRoleController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysRoleBiz csysRoleBiz;
    ...
    @RequestBody CsysRole csysRole) {
        ...
        String insertId=csysRoleBiz.insertDataSettings(baseUserList.get(0),csysRole);
    ...
    @PathVariable String ids) {
        ...
        csysRoleBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysRole csysRole) {
        ...
        csysRoleBiz.updateDataSettings(baseUserList.get(0),csysRole);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysRole csysRole = csysRoleBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysRole,param);
    ...
    @RequestBody CsysRole csysRole) {
        ...
        List<CsysRole> csysRolelst = csysRoleBiz.getDataSettingsByCondition(csysRole);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysRolelst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysRoleBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysRole csysRole) {
		...
		/*--------------end 此段代码可优化成redis-----------------------*/
			...
			System.out.println("baseUserList"+baseUserList.size());
       
         PageInfo pageInfo=csysRoleBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysRole);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
