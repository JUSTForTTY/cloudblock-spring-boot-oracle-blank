...
*/
@RestController
@RequestMapping("/csystrsruledl")
public class CsysTrsRuledlController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysTrsRuledlBiz csysTrsRuledlBiz;
    ...
    @RequestBody CsysTrsRuledl csysTrsRuledl) {
        ...
        String insertId=csysTrsRuledlBiz.insertDataSettings(baseUserList.get(0),csysTrsRuledl);
    ...
    @PathVariable String ids) {
        ...
        csysTrsRuledlBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysTrsRuledl csysTrsRuledl) {
        ...
        csysTrsRuledlBiz.updateDataSettings(baseUserList.get(0),csysTrsRuledl);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysTrsRuledl csysTrsRuledl = csysTrsRuledlBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsRuledl,param);
    ...
    @RequestBody CsysTrsRuledl csysTrsRuledl) {
        ...
        List<CsysTrsRuledl> csysTrsRuledllst = csysTrsRuledlBiz.getDataSettingsByCondition(csysTrsRuledl);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsRuledllst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysTrsRuledlBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysTrsRuledl csysTrsRuledl) {
        ...
        try{
         PageInfo pageInfo=csysTrsRuledlBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysTrsRuledl);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
