...
*/
@RestController
@RequestMapping("/csysexcelimportview")
public class CsysExcelImportViewController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysExcelImportViewBiz csysExcelImportViewBiz;
    ...
    @RequestBody CsysExcelImportView csysExcelImportView) {
        ...
        String insertId=csysExcelImportViewBiz.insertDataSettings(baseUserList.get(0),csysExcelImportView);
    ...
    @PathVariable String ids) {
        ...
        csysExcelImportViewBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysExcelImportView csysExcelImportView) {
        ...
        csysExcelImportViewBiz.updateDataSettings(baseUserList.get(0),csysExcelImportView);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysExcelImportView csysExcelImportView = csysExcelImportViewBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysExcelImportView,param);
    ...
    @RequestBody CsysExcelImportView csysExcelImportView) {
        ...
        List<CsysExcelImportView> csysExcelImportViewlst = csysExcelImportViewBiz.getDataSettingsByCondition(csysExcelImportView);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysExcelImportViewlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysExcelImportViewBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysExcelImportView csysExcelImportView) {
        ...
        try{
         PageInfo pageInfo=csysExcelImportViewBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysExcelImportView);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
