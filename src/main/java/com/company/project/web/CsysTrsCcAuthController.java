...
*/
@RestController
@RequestMapping("/csystrsccauth")
public class CsysTrsCcAuthController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CsysTrsCcAuthBiz csysTrsCcAuthBiz;
    ...
    @RequestBody CsysTrsCcAuth csysTrsCcAuth) {
        ...
        String insertId=csysTrsCcAuthBiz.insertDataSettings(baseUserList.get(0),csysTrsCcAuth);
    ...
    @PathVariable String ids) {
        ...
        csysTrsCcAuthBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CsysTrsCcAuth csysTrsCcAuth) {
        ...
        csysTrsCcAuthBiz.updateDataSettings(baseUserList.get(0),csysTrsCcAuth);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CsysTrsCcAuth csysTrsCcAuth = csysTrsCcAuthBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsCcAuth,param);
    ...
    @RequestBody CsysTrsCcAuth csysTrsCcAuth) {
        ...
        List<CsysTrsCcAuth> csysTrsCcAuthlst = csysTrsCcAuthBiz.getDataSettingsByCondition(csysTrsCcAuth);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(csysTrsCcAuthlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=csysTrsCcAuthBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CsysTrsCcAuth csysTrsCcAuth) {
        ...
        try{
         PageInfo pageInfo=csysTrsCcAuthBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),csysTrsCcAuth);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
