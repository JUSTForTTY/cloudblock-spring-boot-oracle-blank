...
*/
@RestController
@RequestMapping("/cysyslayout")
public class CySysLayoutController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private CySysLayoutBiz cySysLayoutBiz;
    ...
    @RequestBody CySysLayout cySysLayout) {
        ...
        cySysLayoutBiz.insertDataSettings(baseUserList.get(0),cySysLayout);
    ...
    @PathVariable String ids) {
        ...
        cySysLayoutBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody CySysLayout cySysLayout) {
        ...
        cySysLayoutBiz.updateDataSettings(baseUserList.get(0),cySysLayout);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        CySysLayout cySysLayout = cySysLayoutBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(cySysLayout,param);
    ...
    @RequestBody CySysLayout cySysLayout) {
        ...
        List<CySysLayout> cySysLayoutlst = cySysLayoutBiz.getDataSettingsByCondition(cySysLayout);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(cySysLayoutlst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=cySysLayoutBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody CySysLayout cySysLayout) {
        ...
        try{
        Map<String,Object> map=cySysLayoutBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),cySysLayout);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
