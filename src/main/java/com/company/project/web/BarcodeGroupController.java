...
*/
@RestController
@RequestMapping("/barcodegroup")
public class BarcodeGroupController {
	private final Log logger = LogFactory.getLog(getClass());
	
    @Resource
    private BarcodeGroupBiz barcodeGroupBiz;
    ...
    @RequestBody BarcodeGroup barcodeGroup) {
        ...
        String insertId=barcodeGroupBiz.insertDataSettings(baseUserList.get(0),barcodeGroup);
    ...
    @PathVariable String ids) {
        ...
        barcodeGroupBiz.deleteDataSettings(ids);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestBody BarcodeGroup barcodeGroup) {
        ...
        barcodeGroupBiz.updateDataSettings(baseUserList.get(0),barcodeGroup);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @PathVariable String id) {
        ...
        BarcodeGroup barcodeGroup = barcodeGroupBiz.getDataSettings(id);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(barcodeGroup,param);
    ...
    @RequestBody BarcodeGroup barcodeGroup) {
        ...
        List<BarcodeGroup> barcodeGrouplst = barcodeGroupBiz.getDataSettingsByCondition(barcodeGroup);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
        	 
		}
        return ResultGenerator.genSuccessResult(barcodeGrouplst,param);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size) {
        ...
        try{
         PageInfo pageInfo=barcodeGroupBiz.getPageDataSettings(page,size);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
    ...
    @RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer size,@RequestBody BarcodeGroup barcodeGroup) {
        ...
        try{
         PageInfo pageInfo=barcodeGroupBiz.getPageDataSettingsByCondition(page,size,baseUserList.get(0),barcodeGroup);
        if(tokenRefreshFlag) {
        	param.put("access_token", token);
        	param.put("refresh_token", refreshtoken);
