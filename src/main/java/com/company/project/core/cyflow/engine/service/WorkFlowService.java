...
public class WorkFlowService implements WorkflowInterface {
	...
	@Resource
	private CsysTrsAuthViewBiz csysTrsAuthViewBiz;
			...
			if (cySysWorkflow != null) {
				...
				if (csysPot != null) {
					...
					if (csysPotTrs != null) {
						...
						csysWorkflowRunId = insertCsysWorkflowRun(CsysUserView, csysWorkflowRun, cySysWorkflow, csysPot,
								csysPotTrs);
	...
	public String insertCsysWorkflowRun(CsysUserView baseUserView, CsysWorkflowRun csysWorkflowRun,
		...
		// 若工作流承诺办理期限不为空则设置截止时间
		if (cySysWorkflow.getCsysWorkflowDate() != null) {
			// 截止时间（当前迁移节点），即用当前时间加上工作流承诺期限时间（承诺时间单位为天）
			csysWorkflowRun.setCsysWorkflowRunCloseTime(new Date(new Date().getTime()
					+ (int) (cySysWorkflow.getCsysWorkflowDueDate().doubleValue() * 24 * 60 * 60 * 1000)));
		}
		// 开始时间（当前迁移节点）
	...
	@Override
	public JSONObject potStatusTrs(List<CsysUserView> baseUserViewList, CsysWorkflowRun workflowRun) throws Exception {
	...
	private Map<String, Object> judgeIsTrsAuth(List<CsysUserView> baseUserViewList, String cySysWorkflowId,
		...
		map.put("CsysPotTrs", csysPotTrsId != null ? csysPotTrsBiz.getDataSettings(csysPotTrsId) : null);
		map.put("isAutoExe", isAutoExe);
		return map;
	}

	private void insertCsysTrsLog(CsysUserView baseUserView, CsysWorkflowRun csysWorkflowRun, CsysPotTrs csysPotTrs,
			String type) {
		// TODO Auto-generated method stub
	...
	@Override
	public List<CsysTrsAuthView> getTrsStatus(List<CsysUserView> baseUserViewList, CsysTrsAuthView csysTrsAuthView) {
		// TODO Auto-generated method stub
		List<String> roleList = new ArrayList();
		for (CsysUserView baseUserView : baseUserViewList) {
			roleList.add(baseUserView.getCsysRoleId());
